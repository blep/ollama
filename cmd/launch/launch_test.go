...
func TestBuildLauncherState_DeprecatedSavedModelIsUnusable(t *testing.T) {
	tmpDir := t.TempDir()
	setLaunchTestHome(t, tmpDir)
	...
	if state.RunModelUsable {
		t.Fatal("expected deprecated saved run model to be unusable")
	}
	...
	if state.Integrations["codex"].ModelUsable {
		t.Fatal("expected deprecated saved integration model to be unusable")
	}
	if showCalls.Load() != 0 {
		t.Fatalf("deprecated saved models should be rejected from tags metadata without /api/show, got %d calls", showCalls.Load())
	}
}
